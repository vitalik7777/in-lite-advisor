...
export default class StepResolver extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            cssClass: '',
            initStatus: this.props.initialised
        };
    ...
    render() {
        ...
        const lastStep = this.props.steps[this.state.compState].name === this.props.steps.slice(-1).pop().name ? ' last-step' : '';
        const activeStep = this.props.steps[this.state.compState].name;
        const initializedApp = this.state.initStatus ? 'initialized-app ' : '';

        return (
            <div className={initializedApp + "in-lite-advisor" + lastStep + " " + this.state.cssClass}>
